...
class HomeViewModel: ObservableObject {
    ...
    func actionPerformed(action: HomeAction) {
        if case HomeAction.nextPage(let current) = action {
    ...
    private func loadNextPage(current: Character){
        if hasLoadNextPage(current: current){
            data.changePage(newPageNumber: data.page + 1)
            loadCharacters()
        }
    ...
    private func retrieveCharacters(response: CharacterResponse, searchTerm: CurrentValueSubject<String, Never>){
        if let characters = response.data?.results{
            let secure = toHttpsCharactersThumbnail(characters: characters)
    ...
    private func toHttpsCharactersThumbnail(characters: [Character]) -> [Character] {
        return characters.map{
            if($0.thumbnail?.path?.contains("https") == false){
                ...
                let thumbnail = CharacterThumbnail(path: path, thumbnailExtension: $0.thumbnail?.thumbnailExtension)
